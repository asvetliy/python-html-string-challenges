...
**Browser preview:** After opening the HTML file (`html_outputs/challenge_02_output.html`) in a browser, use View Page Source to confirm the href and src values were updated correctly. Your terminal screenshot must show the printed `<link>` and `<script>` lines clearly enough to read the new filenames.
